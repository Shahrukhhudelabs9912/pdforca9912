...
3. Set your password, click **Protect**, and download the secured file
...
1. Generate each employee's salary slip as a PDF
2. Password-protect each file using [Protect PDF](/protect-pdf) — use a convention the employee knows (e.g., date of birth, employee ID)
3. Email the protected PDF
4. Employees enter their password to view
...
1. Protect the PDF with a permissions password that blocks copying and printing
2. Share the readable-only version for feedback
3. Remove protection and send the final version only after payment
...
1. Protect your tax returns, insurance policies, and ID scans
2. Upload the protected versions to your cloud drive
3. Even if someone gains access to your drive, they can't open the protected files

### Sharing Medical Reports

A patient downloads a lab report and needs to share it with a specialist via email:

1. Protect the PDF with a password
2. Email the report to the doctor
3. Share the password via phone or in person
4. The report stays confidential even if the email is compromised
...
If you own the document and know the password, you can remove protection later using the [Unlock PDF tool](/unlock-pdf). This is useful when:
...
The [Protect PDF tool](/protect-pdf) works on any mobile browser:
...
**Can I protect a PDF that's already been shared?**
You can protect your copy going forward, but anyone who already has the unprotected version will still have it. Protection only applies to the new file you create.

**Can recipients remove the password?**
Not without knowing the password. With the open password, they'd need to enter it, and then they could remove it (since they have access). Permissions passwords can theoretically be stripped by some tools, which is why open passwords are the stronger option.

**Does password protection change the file content?**
No. The PDF content stays identical — only the access is gated.

**Can I set different passwords for different people?**
Not in a single PDF. Create separate protected copies with different passwords if needed.

**What if I forget the password?**
There is no recovery mechanism for PDF encryption. If you forget the password, the file is permanently inaccessible. Use a password manager.
...
2. Upload your PDF
3. Set a strong password (8+ characters, mixed)
4. Click **Protect** → download the encrypted file
5. Share the file and the password through **separate channels**
