...
3. Select the pages you want to extract, then click **Split** and download
...
1. Open the [Split PDF tool](/split-pdf)
2. Upload the combined PDF
3. Select the page(s) containing your degree certificate
4. Download the extracted file — upload only what's needed
...
1. Upload the PDF
2. Extract pages 87–112
3. Download a clean 26-page PDF — easier to read, annotate, and share with study partners

### Separating a Signed Page

A client signed page 8 of a 15-page contract. You need just the signed page for your records:

1. Upload the full contract
2. Extract page 8
3. Save separately as `contract-signed-page.pdf`
...
1. Extract pages 1–30 as Part 1
2. Extract pages 31–60 as Part 2
3. Send as two separate emails

Or better yet — [compress the PDF](/compress-pdf) first and see if it fits in one attachment.
...
Both start and end the same way: upload a PDF, pick pages, download the result. The [Split PDF tool](/split-pdf) handles both operations.
...
One tip: if your scan combined two physical pages into one PDF page (e.g., an open book scanned flat), you'll need to crop rather than split. In that case, consider using the [Organize PDF tool](/organize-pdf) to manage pages, or re-scan with one physical page per scan.

## What to Do After Splitting

Once you have the extracted pages, you might want to:

- **Merge** them with other documents — use [Merge PDF](/merge-pdf) to combine extracted pages with new content
- **Compress** the result — run it through [Compress PDF](/compress-pdf) if the extracted file is still too large
- **Rotate** pages — if some extracted pages are sideways, fix them with [Rotate PDF](/rotate-pdf)
- **Add page numbers** — use [Page Numbering](/page-numbering) to add headers or footers to the new document
...
**Compress first, then split.** If the original is huge, [compress it](/compress-pdf) before splitting. The extracted pages will be smaller too.
...
**On iPhone:**
1. Open Safari and navigate to the [Split PDF tool](/split-pdf)
2. Tap upload and select your PDF from Files
3. Choose your pages and split
4. The extracted PDF saves to your Downloads

**On Android:**
1. Open Chrome and go to the Split PDF tool
2. Upload from your file manager
3. Select pages, split, and download
...
First unlock it using the [Unlock PDF tool](/unlock-pdf) (you'll need the password), then split.

**Does splitting change the quality of the pages?**
No. Splitting extracts pages exactly as they are — no re-rendering, no re-compression, no quality loss.

**Can I extract non-consecutive pages (e.g., pages 2, 7, and 15)?**
Yes. Select the individual pages you need and they'll be combined into one new PDF in the order you selected.

**Is there a page limit?**
No hard limit. The tool handles PDFs with hundreds of pages. Very large files may take a bit longer to upload, but the split operation itself is fast.

**Can I rearrange the extracted pages?**
After extracting, use the [Organize PDF tool](/organize-pdf) to drag pages into any order you want.

**Can I split a PDF into equal parts?**
Yes — for example, splitting a 20-page PDF into four 5-page parts. Select each range separately and download individually.
...
4. Click **Split** → download your extracted pages
