...
You have five invoices that a client wants as one attachment. A job application portal that only accepts a single upload. A contract where the signature page came back separately. Whatever brought you here, the fix is the same: merge those PDFs into one file.
...
A wrongly ordered merge is fixable — use the [Organize PDF tool](/organize-pdf) to reorder individual pages after merging — but getting it right the first time saves a step.
...
2. Upload them to the [Merge PDF tool](/merge-pdf)
...
All in one PDF. Upload each document, arrange in the required order, merge, and submit. If individual documents are images, convert them first using the [JPG to PDF tool](/jpg-to-pdf).
...
| Source format | Convert with |
|--------------|-------------|
| Word (.docx) | [Word to PDF](/word-to-pdf) |
| Images (.jpg, .png) | [JPG to PDF](/jpg-to-pdf) |
| Excel (.xlsx) | [Excel to PDF](/excel-to-pdf) |
| PowerPoint (.pptx) | [PowerPoint to PDF](/powerpoint-to-pdf) |
...
1. **Compress it** — Run the merged file through the [Compress PDF tool](/compress-pdf). Medium compression typically cuts size by 50–70% with no visible quality loss.
2. **Remove unnecessary pages** — Use the [Organize PDF tool](/organize-pdf) to delete blank or duplicate pages before compressing.
3. **Split if needed** — If the platform accepts multiple uploads, use the [Split PDF tool](/split-pdf) to break the file into smaller parts.
...
**Check page orientation.** If some pages are rotated sideways after merging, use the [Rotate PDF tool](/rotate-pdf) to fix individual pages.
...
**Bookmark the tool.** If you merge PDFs regularly — for billing cycles, client onboarding, or monthly reports — bookmark the [Merge PDF](/merge-pdf) page for one-click access.

## FAQ

**Can I merge password-protected PDFs?**
Not directly. First, unlock each PDF using the [Unlock PDF tool](/unlock-pdf) (you'll need the password), then merge the unlocked files.

**Does merging change the content or quality of my PDFs?**
No. Merging combines pages sequentially — it doesn't re-render, re-compress, or alter the content. What goes in comes out unchanged, just in one file.

**Can I merge PDFs and images in one step?**
Currently, images need to be converted to PDF first using [JPG to PDF](/jpg-to-pdf), then merged. It takes about 30 extra seconds.

**Is there a file size limit for uploading?**
There's a generous per-file limit for free users. If you're working with very large files (100+ MB each), compress them individually first.

**Can I undo a merge?**
You can't "unmerge," but you can extract specific pages using the [Extract Pages tool](/extract-pages) or split the file using the [Split PDF tool](/split-pdf).

**Does this work without internet?**
No — the tool processes files on the server. You need an active internet connection.
